...
class NumericDelegate(qtw.QStyledItemDelegate):
    ...
    def initStyleOption(self, option: qtw.QStyleOptionViewItem, index: Union[qtc.QModelIndex, qtc.QPersistentModelIndex]) -> None:
        ...
        if self.highlight_empty:
            if '-' in item.text():
                if item.text().split('\n')[-1] == '0':
                    option.backgroundBrush = qtg.QBrush(qtg.QColor(180, 20, 20, 160))
                else:
                    option.backgroundBrush = qtg.QBrush(qtg.QColor(60, 180, 60, 120))
            elif item.text() == '0' and ((index.column() == self.table.columnCount() - 1) or (index.row() == self.table.rowCount() - 1)):
                option.backgroundBrush = qtg.QBrush(qtg.QColor(180, 20, 20, 240))
